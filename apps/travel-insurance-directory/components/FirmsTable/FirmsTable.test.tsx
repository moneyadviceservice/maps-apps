...
describe('FirmsTable', () => {
  ...
  it('renders table with column headers', () => {
    ...
    expect(screen.getAllByTestId('sort-arrow')).toHaveLength(7);
  });
  ...
  it('renders trading name as subsidiary of parent firm name', () => {
    const firmWithTradingName = createMockFirm({
      id: 'firm-3',
      registered_name: 'Just Insurance Agents Limited',
      trading_names: [
        {
    ...
    render(<FirmsTable firms={[firmWithTradingName]} />);
    expect(screen.getByText('- Subsidiary of')).toBeInTheDocument();

    const tradingNameLink = screen.getByRole('link', {
      name: 'Justtravelcover.com',
    });
    const parentNameLink = screen.getByRole('link', {
      name: 'Just Insurance Agents Limited',
    });

    expect(tradingNameLink).toHaveAttribute('href', '/admin/firms/firm-3');
    expect(parentNameLink).toHaveAttribute('href', '/admin/firms/firm-3');

    expect(tradingNameLink).not.toHaveTextContent('Subsidiary of');
    expect(parentNameLink).not.toHaveTextContent('Subsidiary of');
  });
