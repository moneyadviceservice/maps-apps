...
describe('RadioQuestion', () => {
  ...
  it('renders all options with correct labels', () => {
    ...
    expect(screen.queryByTestId('testRadio-title')).not.toBeInTheDocument();
    expect(screen.getByLabelText('Yes')).toBeInTheDocument();
    expect(screen.getByLabelText('No')).toBeInTheDocument();
  });

  it('renders hint text only for options that provide it', () => {
    mockRadioInput.title = 'Choose an option';
    render(<RadioQuestion radioInput={mockRadioInput} initialValue="" />);

    expect(screen.queryByTestId('testRadio-title')).toHaveTextContent(
      'Choose an option',
    );
  ...
  it('applies row layout class when specified', () => {
    ...
    expect(getByTestId('flex-wrapper')).toHaveClass('flex-row');
  });
});
