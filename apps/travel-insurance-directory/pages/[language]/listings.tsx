...
const Page = ({ lang, firms, query, pagination }: BaseProps) => {
  ...
  const { isFilterLoading, onFormChange, onFormBlurCapture } =
    useListingsFilterForm(lang, query);
  ...
  return (
    <ToolPageLayout
      pageTitle={pageTitle(
        z({
          ...
          cy: 'Defnyddiwch ein yfeirlyfr yswiriant teithi',
        }),
        z,
      )}
      title={title}
    ...
    >
      <ListingsLayout
        ...
        onFormBlurCapture={onFormBlurCapture}
      />
    </ToolPageLayout>
  );
};
