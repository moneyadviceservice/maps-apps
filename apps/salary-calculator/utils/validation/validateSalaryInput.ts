...
const NumberOrZero = (value: string | undefined) => (value ? Number(value) : 0);
...
const schema = z
  .object({
    ...
    hoursPerWeek: z
      ...
      .pipe(z.number(HoursPerWeekInvalid)),
    daysPerWeek: z
      .string(DaysPerWeekInvalid)
      .optional()
      .transform(NumberOrZero)
      .pipe(z.number(DaysPerWeekInvalid))
      .default(5),
    taxCode: z
  ...
  .refine(
    ...
    { error: 'days-per-week-invalid', path: ['daysPerWeek'] },
  )
  // Only allow days per week between (inclusive) 1 and 7
  .refine(
    (data) => {
      if (data.grossIncomeFrequency === 'daily') {
    ...
    { error: 'days-per-week-range', path: ['daysPerWeek'] },
  )
  ...
  .refine(
    ...
    { error: 'hours-per-week-invalid', path: ['hoursPerWeek'] },
  )
  // Only allow hours per week between (inclusive) 1 and 168
  .refine(
    (data) => {
      if (data.grossIncomeFrequency === 'hourly') {
  ...
  .refine(
    ...
    { error: 'pension-percent-range', path: ['pensionPercent'] },
  )
  // Only allow fixed pension contribution less than gross income
  .refine(
    (data) => {
      if (typeof data.grossIncome === 'number') {
        return data.pensionFixed < data.grossIncome;
      }

      return true;
    },
    { error: 'pension-fixed-range', path: ['pensionFixed'] },
  )
  // Tax code validation
  .refine(
    (data) => {
