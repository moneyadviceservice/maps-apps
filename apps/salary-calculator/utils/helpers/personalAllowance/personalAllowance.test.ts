...
describe('calculatePersonalAllowance', () => {
  for (const expectation of expectations) {
    const { taxableAnnualIncome, taxYear, isBlindPerson, allowance } =
      expectation;
    test(taxableAnnualIncome.toString(), () => {
      expect(
        calculatePersonalAllowance({
          taxYear: (taxYear ?? '2022/23') as TaxYear,
          taxableAnnualIncome,
          isBlindPerson,
        }),
      ).toEqual(allowance);
  ...
  it('handles negative taxableAnnualIncome and covers negative deduction branch', () => {
    const result = calculatePersonalAllowance({
      taxableAnnualIncome: -5000, // Negative income
      taxYear: '2025/26',
      country: 'England/NI/Wales',
    });
    // Should return full allowance, as deduction is negative and reset to 0
    expect(result).toBe(12570);
  });
});
