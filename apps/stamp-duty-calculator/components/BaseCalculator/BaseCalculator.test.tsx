...
const mockConfig: CalculatorConfig = {
  ...
  instructionsTitle: 'Calculate your test:',
...
describe('BaseCalculator', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should use default instructions when not provided', () => {
      const configWithoutInstructions = {
        ...
        instructionsTitle: undefined,
      };

      render(
        <BaseCalculator {...defaultProps} config={configWithoutInstructions} />,
      );

      expect(screen.getByText(/Select Buyer Type/i)).toBeInTheDocument();
      expect(screen.getByText(/Enter Property Price/i)).toBeInTheDocument();
      expect(screen.getByText(/Press "Calculate"/)).toBeInTheDocument();
    });
  ...
  describe('Focus Management', () => {
    it('should focus on info element when calculated successfully', async () => {
      ...
      await waitFor(() => {
        ...
        // The component calls focus on the element with tabIndex -1
        expect(infoElement).toHaveAttribute('tabindex', '-1');
      });
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle missing validateForm function', () => {
      ...
      render(
        <BaseCalculator
          {...defaultProps}
          config={configWithoutValidation}
          calculated={true}
          initialValues={{ buyerType: 'first', price: '300000' }}
        />,
      );
    ...
    it('should handle function-based instructions and title', () => {
      const configWithFunctions = {
        ...mockConfig,
        instructionsTitle: (_z: any) => 'Function-based title',
        instructions: (_z: any) => <div>Function-based instructions</div>,
      };

      render(<BaseCalculator {...defaultProps} config={configWithFunctions} />);

      expect(screen.getByText('Function-based title')).toBeInTheDocument();
      expect(
        screen.getByText('Function-based instructions'),
      ).toBeInTheDocument();
    });
  ...
  describe('Form Attributes', () => {
    it('should set correct form attributes', () => {
      ...
      const form = container.querySelector('form')!;
      expect(form.id).toBe('test-calculator');
      expect(form.method).toBe('get');
    });

    it('should handle form name with spaces', () => {
      const configWithSpacedName = {
        ...mockConfig,
        name: 'My Spaced Calculator Name',
      };

      const { container } = render(
        <BaseCalculator {...defaultProps} config={configWithSpacedName} />,
      );

      const form = container.querySelector('form')!;
      expect(form.id).toBe('my-spaced-calculator-name');
    });
  });
  ...
  describe('Button States', () => {
    it('should show Calculate button when not calculated', () => {
      ...
      expect(button.className).toBe('test calculator-calculate');
    });
    ...
    it('should show Recalculate button when calculated', () => {
      ...
      expect(button.className).toBe('test calculator-recalculate');
    });
  });
