...
import ChevronRight from '@maps-react/common/assets/images/chevron-right.svg';
...
export const AccountPagination = ({
  ...
  const nextPrevClasses = [
    'border-4 rounded-md text-pink-600 visited:!text-pink-600',
  ];
        ...
        <ul className="flex items-center justify-center flex-1 min-w-full space-x-1 lg:flex-none lg:min-w-fit">
          {pages.map((i, index) => {
            ...
            return (
              <li key={index}>
                {addDotsToStart && (
                  <span key={index} className="mr-2 text-pink-600 md:hidden">
                    &#8230;
                  </span>
                )}
                <Link
