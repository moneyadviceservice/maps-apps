...
export const PensionLossAppointment: StepComponent = ({ step }) => {
  const { t, tList } = useTranslation();
  const componentKey = 'components.pension-loss-appointment';
    ...
    <>
      <div className="flex flex-col gap-4">
        ...
        <SectionsRenderer
          ...
          testIdPrefix="pension-loss-appointment"
        />
        <H2 className="text-blue-700">{t(`${componentKey}.callout.title`)}</H2>
        <Callout>
          <SectionsRenderer
            sections={calloutSections}
            testIdPrefix="pension-loss-appointment-callout"
          />
        </Callout>
      </div>
      <FormWrapper step={step}>
        <input type="hidden" name="mainFlow" value={FlowName.PENSION_LOSS} />
      </FormWrapper>
    </>
  );
};
