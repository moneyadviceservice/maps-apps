...
export function validateFormSubmission(
  ...
  if (!parsedData.success) {
    return parsedData.error.issues.map((err) => ({
      field: err.path[0] as string, // The field name (e.g., "first-name")
      message: err.message, // The error message used to reference the language file to enable multi-language support
    }));
  }
