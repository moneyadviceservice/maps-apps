...
export function validateFormSubmission(
  ...
  const dataObjectWithFlow = {
    ...dataObject,
    flow: entry.data.flow, // Include the flow as it is used for conditional validation based on flow type (See ContactDetails.tsx)
  };

  // Validate the form data using the schema and return errors if any
  const parsedData = validationSchema.safeParse(dataObjectWithFlow);
