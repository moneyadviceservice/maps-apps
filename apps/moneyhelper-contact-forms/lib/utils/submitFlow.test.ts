...
describe('submitFlow utils', () => {
  ...
  it('runSubmitFlow handles API non-ok and redirects to error with status code', async () => {
    fetchSpy.mockResolvedValue({
      ok: false,
      status: 500,
  ...
  it('runSubmitFlow falls back to GENERIC_ERROR for non-numeric network errors', async () => {
