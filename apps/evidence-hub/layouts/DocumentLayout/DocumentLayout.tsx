...
function getSectionData(page: DocumentTemplate) {
  return {
    ...page,
...
export const DocumentLayout = ({ page }: Props) => {
  const sectionData = getSectionData(page);

  const links = sectionData.sections?.map((section) => {
    return {
    ...
    <Container className="max-w-[1272px]">
      ...
      >
        <div className="pb-16 basis-2/3">
          ...
          <aside
            className="border-l-8 border-teal-300 py-4 pl-6 pr-0 mb-14"
            data-testid="evidence-type"
          >
            Evidence type:{' '}
            <span className="text-pink-600" data-testid="evidence-type-data">
              {page.pageType?.name}
            </span>
          ...
          {links && (
            <>
              <p className="mb-5 font-bold">On this page</p>
              <ul
                ...
                {links?.map(({ linkTo, text }) => (
                  <li key={text} className="border-b-1 border-slate-400">
                    <Link
                      href={linkTo}
                      className={twMerge([
                        'no-underline text-pink-600 py-[6px] w-full',
                      ])}
                    >
                      {text}
                    </Link>
          ...
          <DocumentSections sections={sectionData.sections} />
        </div>
