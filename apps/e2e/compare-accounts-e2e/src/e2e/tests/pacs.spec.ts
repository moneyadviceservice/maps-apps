...
test('Sort results by Bank name Z-A', async ({ page }) => {
  ...
  const firstAccount = page
    .locator('div[data-testid="selected-accounts"]')
    .first();
  await expect(firstAccount.locator('h3')).toContainText('Virgin Money');
});
...
test('Sort results by Minimum monthly deposit (lowest first)', async ({
  ...
  // Wait for the sorting dropdown to be ready
  const sortDropdown = page.locator('select#order');
  await sortDropdown.waitFor({ state: 'visible' });

  // Select the sorting option
  await sortDropdown.selectOption('minimumMonthlyDepositLowestFirst');
