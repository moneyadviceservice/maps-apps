...
describe('Cash in chunks calculator', () => {
  beforeEach(() => {
    cy.skipExceptions();
    cy.setBreakPoint('desktop');
    cy.setCookieControl();
    cy.visit('/en/cash-in-chunks');
  });
...
describe('Embed Cash In chunks calculator', () => {
  beforeEach(() => {
    cy.skipExceptions();
    cy.setBreakPoint('desktop');
    cy.setCookieControl();
    cy.visit('/en/cash-in-chunks?isEmbedded=true');
  });

  const page = new cashChunks();

  it('should have isEmbedded query param and no header and footer', () => {
    const data = cash.tests[0];
    page.enterIncomeValue(data.income);
    page.enterCashInChunks(data.cashInChunk);
