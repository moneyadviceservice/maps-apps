...
describe('Budget Planner End to End', () => {
  ...
  const confirmSelectedNavItem = (step: string) => {
    ...
    cy.get(
      `button[formaction="/api/budget-planner/${step}"]${selected_step_class}`,
    );
  };
  ...
  beforeEach(() => {
    ...
    cy.visit('/en/budget-planner/income');
  });
...
*/
  it('Balance should change colours', () => {
    const pages = ['/en', '/cy'];
    pages.forEach((page) => {
      cy.visit(`${page}/budget-planner/income`);
  ...
  it('Save and Return Journey - Valid email', () => {
    const save = 'save';
    cy.intercept(`/**/en/budget-planner/${save}*`).as(save);
    ...
    cy.fixture('progressSaved.html').then((progressSaved) => {
      cy.intercept('POST', `/api/budget-planner/${SaveReturn}*`, (req) => {
        req.reply({
          statusCode: 200,
          body: progressSaved,
        });
  ...
  it('Save and Return Journey - Invalid email', () => {
    cy.intercept('POST', '/api/budget-planner/save-and-return', (req) => {
      req.reply((res) => {
        res.send({
