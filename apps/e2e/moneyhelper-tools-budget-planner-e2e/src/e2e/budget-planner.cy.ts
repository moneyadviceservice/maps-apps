...
describe('Budget Planner End to End', () => {
  ...
  const confirmSelectedNavItem = (step: string) => {
    const selected_step_class = '.text-blue-800';
...
*/
  ...
  it('Save and Return Journey - Valid email', () => {
    const save = 'save';
    const SaveReturn = 'save-and-return';

    // Set up intercepts before navigation
    // Intercept the POST to the API that happens when clicking save button
    cy.intercept('POST', '/api/household-bills*', (req) => {
      // Mock the redirect response
      req.reply({
        statusCode: 303,
        headers: {
          Location: `/en/save?save=true&tabName=income&isEmbedded=false&language=en`,
        },
      });
    }).as('savePost');

    // Intercept the save page load
    cy.intercept('GET', `/**/en/${save}*`).as(save);

    // Intercept the save and return API
    cy.fixture('progressSaved.html').then((progressSaved) => {
      cy.intercept('POST', `/api/${SaveReturn}*`, (req) => {
        req.reply({
          statusCode: 200,
          body: progressSaved,
        });
    ...
    // The form should submit and redirect to save page
