...
describe('Guaranteed Income', () => {
  ...
  it('Recalculate', () => {
    ...
    page.validateResults('90,000', '£22,500 tax free', '4,200');

    page.enterPotIncome('80,000');
    page.elements.calculate().click();
    page.validateResults('80,000', '£20,000 tax free', '3,700');
  });
});

describe('Embed Guaranteed Income calculator', () => {
  beforeEach(() => {
    cy.skipExceptions();
    cy.setBreakPoint('desktop');
    cy.setCookieControl();
    cy.visit('/en/guaranteed-income-estimator?isEmbedded=true');
  });
