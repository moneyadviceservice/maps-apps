...
describe('Embed Take whole Pot calculator', () => {
  beforeEach(() => {
    cy.skipExceptions();
    cy.setBreakPoint('desktop');
    cy.setCookieControl();
    cy.visit('/en/take-whole-pot?isEmbedded=true');
  });
  ...
  it('should have isEmbedded query param and no header and footer', () => {
    ...
    page.enterPotValue(data.takeHome);
    page.enterYearlyIncome(data.income);
