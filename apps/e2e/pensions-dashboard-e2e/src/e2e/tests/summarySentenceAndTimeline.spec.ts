...
import Timeline, {
  ...
  retirementDateMap,
} from '../pages/components/Timeline';
...
test.describe('Confirmed Pensions Summary Section tests', () => {
  ...
  test('Summary Sentence - State Pension Year and ERI available, and timeline page automation', async ({
    ...
    const tooltipInput = page.locator('[data-testid="tooltip-input"]');
    ...
    const actualTimelineData = await timeline.getTimelineData(page);
    expect(actualTimelineData).toEqual(expectedTimelineData);

    // Verify accordion pension count matches data length
    for (const expectedEntry of expectedTimelineData) {
  ...
  test('Summary Sentence - State Pension Year available but no benefit illustration available', async ({
    page,
  }) => {
    await commonHelpers.navigatetoPensionsFoundPage(
      page,
      'StatePensionSummaryNoSPBenefitIllustration2',
    );
  ...
  test('Summary sentence not available when there is no State Pension and no pensions with an estimated income', async ({
    ...
    await commonHelpers.navigatetoPensionsFoundPage(page, 'greenNoIncome');
