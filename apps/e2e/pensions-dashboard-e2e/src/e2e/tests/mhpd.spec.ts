...
test.describe('Supported Pensions - Verify components and information', () => {
  ...
  test(
    ...
    async ({ page, isMobile }) => {
      ...
      await expect(basePage.getHomeLink(page)).toBeVisible();
      ...
      await expect(pensionsFoundPage.areYouExpectingHeader(page)).toHaveText(
        expectedText.areYouExpectingToSeeOtherPensions,
      );
      await pensionsFoundPage.pensionsNotShowingLink(page).click();
  ...
  test('Pensions that need action (red)', async ({ page }) => {
    ...
    await expect(pensionsThatNeedAction.getParagraph1(page)).toBeVisible();
    await expect(pensionsThatNeedAction.getParagraph2(page)).toBeVisible();
    await expect(pensionsThatNeedAction.getParagraph3(page)).toBeVisible();
    await expect(pensionsThatNeedAction.getParagraph4(page)).toBeVisible();
    await expect(
      pensionsThatNeedAction.getPossibleMatchHeading(page),
    ).toBeVisible();
    await expect(
      pensionsThatNeedAction.getPossibleMatchParagraph(page),
    ).toBeVisible();
    await expect(pensionsThatNeedAction.getMoreInfoHeading(page)).toBeVisible();
    await expect(
      pensionsThatNeedAction.getMoreInfoParagraph(page),
    ).toBeVisible();
    ...
    // In your test:
    const items = pensionsThatNeedAction.getListItems(page);
...
test.describe('Home page link', () => {
  ...
  test('should navigate to home page when home link is clicked', async ({
    ...
    const assertHomeLinkOnlyIsVisible = async () => {
      await expect(basePage.getHomeLink(page)).toBeVisible();
      await expect(basePage.getBackLink(page)).toBeHidden();
    };
    ...
    await basePage.checkHomeNavigation(page);
    expect(page.url()).toContain(URL.YOUR_PENSION_SEARCH_RESULTS);

    // 2. Timeline page
    await pensionsFoundPage.clickSeeYourPensions(page);
    await pensionBreakdownPage.clickTimelineLink(page);
    ...
    await pensionsThatNeedAction.proceedToRedTrafficPensionDetailsPage(page);
    await assertHomeLinkOnlyIsVisible();
    await basePage.checkHomeNavigation(page);
    expect(page.url()).toContain(URL.YOUR_PENSION_SEARCH_RESULTS);
