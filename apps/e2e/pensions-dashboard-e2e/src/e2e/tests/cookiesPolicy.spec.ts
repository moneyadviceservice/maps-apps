...
const expectedSections = [
  ...
  },
  {
    ...
    text: `2. Strictly necessary cookies Essential cookies are needed to make the site work, and to keep your information secure. If you do not want us to use them, you can turn off cookies in your web browser, but the service may not work properly. We will not set any essential cookies until you access the website and start using the service. Security cookies Name Purpose Expires codeVerifier This cookie helps the website confirm that the part you see (the front end) is properly connected to the behind-the-scenes services (the back end). Session expiry Functional cookies Name Purpose Expires authorizationCode This cookie stores the authorisation code from the Find your pensions service after One Login verification process. Session expiry beaconId This cookie is used to identify the session anonymously and check that your device is allowed to access the service. This cookie expires automatically after 90 days. userSessionId This cookie tracks the individual session identity of the user. Session expiry redirectUrl This cookie is used to redirect the user to the Find service from One Login. Session expiry currentUrl This cookie is used to keep track of the current page the user is on to enable the user to use the back links. Session expiry supportCurrentUrl This cookie is used to track the page the user is on to enable the user to use the back links on the support pages. Session expiry channel This cookie is used to track the page the user is on to enable the user to use the back links when they access different channels of information from the pensions found page. Session expiry pensionID This cookie is used to track the pension the user is viewing when they access a pension details page. Session expiry Back to top`,
  },
  {
