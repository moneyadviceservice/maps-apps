...
test.describe('Pension Details page - Income and Values Tab - DB pension Accordions', () => {
  ...
  test('How these values are calculated accordion displays BS message', async ({
    ...
    const expectedTexts = [
      'Estimates for this defined benefit pension are based on your salary and years you’ve been a scheme member, as well as the expected retirement date. They’re also based on the provider’s assumptions about inflation and whether the scheme is active. \nTo help you understand the impact of inflation, the estimated income is shown in today’s money, so you can see what that amount would be worth right now.',
    ];
