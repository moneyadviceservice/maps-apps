...
import { PensionType } from '../data/barChartScenario.types';
import {
  lumpSumScenario,
...
test.describe('Pension Details page - Income and Values Bar Chart/Doughnut Chart', () => {
  ...
  test('Bar chart is rendering the correct dates, values and sizes across all scenarios', async ({
    page,
  }) => {
    await commonHelpers.navigatetoPensionsFoundPage(
      page,
      newDetailsPageSummary.option,
    );
    await pensionsFoundPage.clickSeeYourPensions(page);

    for (const pension of newDetailsPageSummary.pensions) {
      ...
      const buildExpectedLabel = (monthly: string, yearly: string) => {
        if (!monthly) return 'Unavailable';
        if (!yearly) return 'Unavailable';
        return `${yearly} a year\n${monthly} a month`;
      };

      const expectedLatestValueLabel = buildExpectedLabel(
        latestMonthly,
        latestYearly,
      );

      const expectedRetirementLabel = buildExpectedLabel(
        retirementMonthly,
        retirementYearly,
      );
  ...
  test('Doughnut chart is rendering the correct dates, values and sizes across all scenarios', async ({
    ...
    for (const pension of newDetailsPageSummary.pensions) {
      ...
      const pensionCardType = await pensionBreakdownPage
        .getPensionCardType(page, pension.schemeName)
        .innerText();

      // Doughnut chart is only available on DC pensions
      if (pensionCardType == 'Defined contribution pension') {
        ...
        const potValueToolTipIconState = page.locator(
          'p:has-text("Pot value") input[data-testid="tooltip-input"]',
        );
  ...
  test('Doughnut chart - AP is greater than ERI value / AP is equal to ERI value', async ({
    ...
    await commonHelpers.navigatetoPensionsFoundPage(page, dataScenario2.option);
    await pensionsFoundPage.clickSeeYourPensions(page);
    ...
    for (const pension of dataScenario2.pensions) {
      ...
      // Get colours from webpage
      const leftDoughnutColours = await doughnutChart.getColoursFromDom('left');
      const rightDoughnutColours = await doughnutChart.getColoursFromDom(
        'right',
      );
      ...
      // check variables
      expect(leftDoughnutColoursPath).toEqual(leftDoughnutColoursFromData.path);
      expect(leftDoughnutColoursOuter).toEqual(
        leftDoughnutColoursFromData.outer,
      );
      expect(leftDoughnutColoursInner).toEqual(
      ...
      if (!latestValue) {
        ...
        expectedLatestValueAmount =
          await pensionDetailsPage.formatCurrencyStringWithCommas(latestValue);
      }
