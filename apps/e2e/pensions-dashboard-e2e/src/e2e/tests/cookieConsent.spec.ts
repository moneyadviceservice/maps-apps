...
test.describe('Cookie Consent Functionality', () => {
  test.beforeEach(async ({ page, context }) => {
    await commonHelpers.navigateToStartPageWithoutCookieConsent(page);
  });
  ...
  test('User can access analytics preferences via footer link', async ({
    ...
    const cookiesPreferencesSideBanner = new CookiesPreferencesSideBanner(page);
    await expect(
      cookiesPreferencesSideBanner.getCookiePolicyLink(),
    ).toBeVisible();
    const popupPromise = page.waitForEvent('popup');
    await cookiesPreferencesSideBanner.getCookiePolicyLink().click();
  ...
  test('Cookie footer link directs to cookie policy page', async ({ page }) => {
    const cookieBanner = page.getByRole('dialog', {
      name: 'Cookies on MoneyHelper Pensions Dashboard',
    });
    await expect(cookieBanner).toBeVisible({ timeout: 3000 });
    ...
    await footer.getCookiesLink().click();
