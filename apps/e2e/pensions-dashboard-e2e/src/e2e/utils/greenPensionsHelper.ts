...
const findMatchingArrangement = (pensionPolicies, schemeNameOnCard) =>
  pensionPolicies
    .flatMap((p: any) => p.pensionArrangements)
    .find((a: any) => a.schemeName === schemeNameOnCard);

const findComponent = (matchingArrangement, illustrationType) =>
...
export async function verifyGreenPensions(page: Page, pensionPolicies: any) {
  ...
  for (const pensionCard of pensionCards) {
    ...
    switch (caseType) {
      ...
      case 'DBWithEstimatedIncome': {
        ...
        expect(pensionTypeText).toContain(pensionTypeData.toLowerCase());
        expect(pensionProviderText).toContain(pensionProviderData);
    ...
    if (type === 'SP') {
      ...
      const statePensionSubtextData = spSubtext.replace(/\s+/g, ' ').trim();
      ...
      //const pensionWithEstimatedIncomeSubtext = `This is a ${pensionTypeMap} workplace pension. The amount you're paid is based on your salary and years you've been a scheme member. This could give you an income of ${formattedEriMonthlyAmount} a month from the pension retirement date of ${formattedRetirementDate}.`;

      const pensionWithEstimatedIncomeSubtextDc = `In this pension pot£${apLumpSumAmountData} You could receive £${formattedEriMonthlyAmount} a month from the first payable date of ${formattedRetirementDate}.`;
      const pensionWithEstimatedIncomeSubtextDbBase = `You could receive£${formattedEriMonthlyAmount} a monthfrom the first payable date of ${formattedRetirementDate}.`;
      ...
      if (hasIncome) {
        if (type === 'DB') {
          ...
          expect(pensionDetailsSubtext).toContain(
            pensionWithEstimatedIncomeSubtextDbBase,
          );
          // Check for lump sum text if it should be present
          ...
          expect(pensionDetailsSubtext).toContain(
            pensionWithEstimatedIncomeSubtextDc,
          );
        }
      }
      ...
      const doughnutToolTipText =
        'This defined benefit pension has a lump sum. This is a one-time sum of money paid in addition to your pension income, usually when you retire. It’s based on your salary and number of years you’ve been in the pension scheme.';
      const accordionText =
        'Estimates for this defined benefit pension are based on your salary and years you’ve been a scheme member, as well as the scheme’s retirement date. They’re also based on the provider’s assumptions about inflation and whether the scheme is active.';
      const barChartAPYearlyAmountText = `£${formatAmountNoPennyForDisplay(
        apAnnualAmountData,
      )} a year`;
      const barChartAPMonthlyAmountText = `£${formatAmountForDisplay(
        apMonthlyAmountData,
      )} a month`;
      const barChartERIMonthlyAmountText = `£${formatAmountNoPennyForDisplay(
        eriMonthlyAmountData,
      )} a month`;
      ...
      if (apPayableDetails === null && eriPayableDetails === null) {
      ...
      if (
        ...
        const doughnutLegendDataERI =
          await pensionDetailsPage.getTextFromLocator(page, 'donut-legend-1');
        expect(lumpSumSentence).toContain(doughnutToolTipText); // This is now INSIDE the if block
        expect(doughnutLabelDataAP).toContain(
        ...
        expect(doughnutLegendDataERI).toContain(retirementYear);
        expect(doughnutAccordionText).toContain(accordionText);
      }
      ...
      // Assertions
      expect(pensionProviderDisplayedText.toLowerCase()).toBe(
      ...
      if (await phoneLocator.isVisible()) {
        expect(await phoneLocator.innerText()).toContain(
          expectedContactData.phone,
        );
      }
      ...
      if (await postalAddressLocator.isVisible()) {
        ...
        const formattedPostalAddress = expectedPostalAddress
          .replace(/·\n\s*/g, ', ')
          .replace(/·/g, '')
          .replace(/\n\s*/g, ', ')
          .replace(/\s+,/g, ',')
        ...
        const stripCountry = (s: string) =>
          s.replace(/,\s*(united kingdom|uk)$/i, '').trim();

        expect(stripCountry(formattedPostalAddress)).toContain(
          stripCountry(expectedContactData.address),
        );
      }
    }
  ...
  await commonHelpers.clickBackLink(page);
  await page.locator('text=See details').waitFor({ state: 'visible' });
}
