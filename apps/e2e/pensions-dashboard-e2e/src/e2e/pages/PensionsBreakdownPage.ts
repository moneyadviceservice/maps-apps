...
type PensionBreakdownPage = {
  ...
  getPensionProvider(page: Page, schemeName: any): any;
  ...
  clickSeeDetailsButton(page: Page, pensionCard: any): Promise<void>;
  clickPensionsTimelineButton(page: Page, locale?: Locale): Promise<void>;
  ...
  getLinkedPensions(page: Page, schemeName: any): any;
  getLinkedPensionsList(page: Page): any;
  getLinkedPensionsToolTipText(page: Page): any;
  ...
  clickCBLumpSumTooltip(page: Page): Promise<void>;
  getCBLumpSumTooltipText(page: Page): Promise<string>;
  cbLumpSumTooltipIcon(page: Page): Locator;
  clickTimelineLink(page: Page): Promise<void>;
  understandYourNextStepsHeading(page: Page): Locator;
  understandYourNextStepsContent(page: Page): Locator;
  clickRetirementGuidanceLink(page: Page): Promise<Page>;
};
...
const pensionBreakdownPage: PensionBreakdownPage = {
  ...
  notIncludedText: `Some pensions may not display an estimated income, such as pensions less than 2 years from their expected retirement date or schemes in the process of winding up. See the pension’s details for more information.`,
  ...
  getPensionProvider(page, schemeName) {
  ...
  async viewDetailsOfPension(page, schemeName): Promise<void> {
    ...
    const matchingScheme = pensionCards
      ...
      .filter({ has: page.getByText(schemeName, { exact: true }) });
  ...
  async clickPensionsTimelineButton(
    page: Page,
    locale: Locale = 'en',
  ): Promise<void> {
    const timelineLinks = {
    ...
    await page.getByRole('link', { name: timelineLinks[locale] }).click();
  ...
  async assertPensions(page, pensions): Promise<void> {
    ...
    for (const pension of pensions) {
      if (
        ...
        (pension.matchType === 'DEFN' &&
          ['CDC', 'CB'].includes(pension.pensionType)) ||
        (pension.matchType === 'DEFN' &&
          ['DC', 'DB', 'SP', 'HYB', 'AVC'].includes(pension.pensionType) &&
    ...
    for (const pension of confirmedPensions) {
      await confirmedPensionsSection.scrollIntoViewIfNeeded();
    ...
    for (const pension of noIncomePensions) {
      await noIncomePensionsSection.scrollIntoViewIfNeeded();
  ...
  getActiveStatus(page, schemeName) {
    return this.getPensionCard(page, schemeName).getByTestId('pension-status');
  },
  ...
  getLinkedPensions(page, schemeName) {
    return this.getPensionCard(page, schemeName).getByTestId(
      'pension-card-linked-pensions',
    );
  },

  async getLinkedPensionsList(page) {
    const linkedPensions = await page
      .getByTestId('pension-detail-linked')
      .innerText();
    return linkedPensions.split('\n').map((pension) => pension.trim());
  },

  async getLinkedPensionsToolTipText(page) {
    const tooltipText = await page.getByTestId('').innerText();
  ...
  async clickCBLumpSumTooltip(page) {
    await page.getByTestId('donut-heading').getByTestId('tooltip-icon').click();
  },

  async getCBLumpSumTooltipText(page) {
    return (
      await page
  ...
  cbLumpSumTooltipIcon(page) {
    return page.getByTestId('donut-heading').getByTestId('tooltip-icon');
  },

  async clickTimelineLink(page: Page) {
    await page.getByTestId('timeline-link').click();
    await page.waitForURL('**/your-pensions-timeline');
  },

  understandYourNextStepsHeading(page: Page): Locator {
    return page.getByTestId('onward-journey-banner-heading');
  },

  understandYourNextStepsContent(page: Page): Locator {
    return page.getByTestId('onward-journey-banner-body');
  },

  async clickRetirementGuidanceLink(page: Page): Promise<Page> {
    const link = page.getByRole('link', { name: 'Get retirement guidance' });
    return await commonHelpers.clickLinkAndReturnNewPage(page, link);
  },
};
