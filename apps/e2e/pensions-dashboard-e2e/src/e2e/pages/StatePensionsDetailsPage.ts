...
type StatePensionsDetailsPage = {
  ...
  getGuideTextOnAPIncome: (page: Page) => Promise<string | null>;
  getDisplayedAPAmount: (page: Page) => Promise<string | null>;
  getGuideTextOnERIIncome: (page: Page) => Promise<string | null>;
  getDisplayedERIAmount: (page: Page) => Promise<string | null>;
...
const statePensionsDetailsPage: StatePensionsDetailsPage = {
  ...
  async getGuideTextOnAPIncome(page: Page) {
    return (
      (
        await page.getByTestId('sp-estimated-income-ap').textContent()
      )?.trim() ?? ''
    );
  },

  async getDisplayedAPAmount(page: Page) {
    return (
      (await page.getByTestId('sp-progress-bar-ap').textContent())?.trim() ?? ''
    );
  },

  async getGuideTextOnERIIncome(page: Page) {
    return (
      (
        await page.getByTestId('sp-estimated-income-eri').textContent()
      )?.trim() ?? ''
    );
  },

  async getDisplayedERIAmount(page: Page) {
    return (
      (await page.getByTestId('sp-progress-bar-eri').textContent())?.trim() ??
      ''
    );
  },
