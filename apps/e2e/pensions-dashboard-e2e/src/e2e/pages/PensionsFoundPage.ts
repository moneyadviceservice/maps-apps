...
const pensionsFoundPage: PensionsFoundPage = {
  ...
  async getPensionChannelByName(
    ...
    const targetPensionListItem = allPensionListItems.filter({
      hasText: new RegExp(`^${pensionName}$`), // Use Regular Expression to ensure it's an exact match.
    });

    // Get the grandparent of the element (should be the callout container)
    const parentCalloutContainer = targetPensionListItem.locator('../..');
    const containerName = await parentCalloutContainer.getAttribute(
      'data-testid',
    );
