    ...
    pages.forEach((url) => {
      ...
      cy.get('.t-recommended-reading').within(() => {
        cy.get('details').each(($elem) => {
          ...
          cy.wrap($elem).find('div').should('have.css', 'font-size', '19px');
          cy.wrap($elem).click();
      ...
      cy.get('.tool-expand > .flex > .block')
        .eq(0)
        .then(($elem: any) => {
          cy.checkCSS($elem, 'color', 'rgb(174, 0, 96)');
          ...
          cy.wrap($elem)
            ...
            .then(($parent: any) => {
              cy.confirmAttributeValue(
                $parent,
                'class',
                'underline hover:text-pink-800 hover:no-underline',
              );
            });
        });
    });
 ...
 */
  ...
  it('Handles error', () => {
    ...
    for (const [url, text] of Object.entries(locales)) {
      ...
      cy.elementContainsText('.text-lg > a', text[1]);
      cy.elementContainsText('#priceIdError div', text[1]);

      cy.get('.t-error-summary [data-testid="icon-error"]').should('exist');
      ...
      cy.get('.text-lg > a')
        .should('have.css', 'color', 'rgb(208, 0, 0)')
        .should('have.css', 'text-decoration-line', 'underline')
        .should('have.attr', 'href')
        .and('contain', `${url}`);
      cy.get('#priceIdError div').should('have.css', 'color', 'rgb(208, 0, 0)');
    }
  });
  //------------------------------------------------------------------------------------------------------------------
  ...
  it('Displays teaser cards', () => {
    ...
    pages.forEach((url) => {
      ...
      cy.get('[data-testid="teaserCard"]').each(($elem: any) => {
        cy.wrap($elem).should('have.attr', 'href');
        cy.checkCSS($elem, 'outline-color', 'rgb(157, 161, 202)');
        cy.checkCSS($elem, 'border-radius', '0px 0px 0px 36px');
        cy.checkCSS($elem, 'color', 'rgb(200, 42, 135)');
        cy.wrap($elem)
          .find('img')
          .then(($img: any) => {
            cy.wrap($img).should('be.visible');
            cy.get($img).invoke('width').should('be.lt', 408);
            cy.get($img).invoke('height').should('be.lt', 205);
          });

        cy.wrap($elem)
          .find('h3')
          .then(($h3: any) => {
            cy.checkCSS($h3, 'color', 'rgb(200, 42, 135)');
            cy.checkCSS($h3, 'font-size', '28px');
          });

        cy.wrap($elem)
          .find('p')
          .then(($p: any) => {
            cy.checkCSS($p, 'color', 'rgb(0, 11, 59)');
            cy.checkCSS($p, 'font-size', '18px');
          });
      });
    });
  });
