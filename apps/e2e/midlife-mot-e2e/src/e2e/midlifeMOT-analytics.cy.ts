...
describe('MidLifeMOT-Analytics', () => {
  ...
  it('DataLayer - pageLoadReact', () => {
    ...
    const generateUrls = (): string[] => {
      ...
      for (let i = 1; i <= questionCount; i++) {
        ...
        urls.push(`${basePath}question-${i}${query}`);
      }

      const finalQuery = buildQuery(questionCount);
      urls.push(`${basePath}change-options?${finalQuery}`);
      urls.push(`${basePath}summary?${finalQuery}`);
      urls.push(`${basePath}results?${finalQuery}`);
  ...
  it('DataLayer - toolStart', () => {
    const locales = ['en', 'cy'];
    const urls = ['mid-life-mot/question-1'];
    locales.forEach((locale) => {
  ...
  it('DataLayer - toolRestart', () => {
    const locales = ['en', 'cy'];
    const urls = ['mid-life-mot/question-1?restart=true'];
  ...
  it('DataLayer - errorMessage', () => {
    const locales = ['en', 'cy'];
    const urls = ['mid-life-mot/question-1?error=q-1'];
