...
Cypress.Commands.add(
  ...
  (
    ...
    if (hasMockedAPI)
      cy.intercept(
        {
          pathname: '/api/start-task',
          headers: { referer: /.*\/pension-wise-appointment.*&t11=4.*/ },
        },
        (req) => {
    ...
    if (hasMockedAPI)
      cy.wait('@startTask').then(() => {
        cy.visit(
          `/en/pension-wise-appointment/summary${query?.replace(
            '/',
            '',
          )}&complete=true&urn=PUU9-3EEE`,
        );
        cy.log('WAIT ....');
      });
