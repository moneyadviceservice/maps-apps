describe('Pension Appointment Pension Options Section', () => {
  ...
  it(`completes save and come back later feature`, () => {
    ...
    cy.get(`h1[data-testid='section-title']`).scrollIntoView();
    cy.get(`h1[data-testid='section-title']`).should('be.visible');

    cy.intercept(`/en/pension-wise-appointment/progress/${progressSaved}*`).as(
      progressSaved,
    );
    ...
    cy.wait('@saveAndReturn').then(() => {
      cy.visit(`/en/pension-wise-appointment/progress/${progressSaved}`);
    });
    ...
    cy.get('[data-testid="progress-saved-primary"]').should('be.visible');
    cy.get('[data-testid="resend-email"]').should('be.visible');
  });

  it(`completes get next steps journey`, () => {
    const savedNextSteps = 'saved-next-steps';
    cy.intercept('POST', `/api/save-and-return`, (req) => {
      req.reply({ statusCode: 200 });
    }).as('saveAndReturn');
    ...
    cy.intercept(`/en/pension-wise-appointment/progress/${savedNextSteps}*`).as(
      savedNextSteps,
    );
    ...
    cy.get(`button[type='submit'][data-testid='save-and-return']`).click();
    cy.wait('@saveAndReturn').then(() => {
      cy.visit(`/en/pension-wise-appointment/progress/${savedNextSteps}`);
    });
    cy.wait(`@${savedNextSteps}`).its('response.statusCode').should('eq', 200);
    ...
    cy.get('[data-testid="progress-saved-primary"]')
      .should('be.visible')
      .should('have.attr', 'href')
      .and('match', /en\/pension-wise-appointment\/summary/);

    cy.get('[data-testid="resend-email"]')
      .should('be.visible')
      .should('have.attr', 'href')
      .and('match', /en\/pension-wise-appointment\/get-next-steps/);
  });
});
