describe('Pension Appointment Pension Options Section', () => {
  ...
  it('shows embedded journeys - calculators', () => {
    ...
    const calculators = [
      ['Take your whole pot', 'take-pot-in-one', 'take-whole-pot-calculator'],
      ['Leave Pot untouched', 'retire-later-or-delay', 'leave-pot-untouched'],
      ['Take Cash in chunks', 'lump-sums', 'cic-calculator'],
      ['Adjustable income', 'flexible-income', 'adjustable-income-estimator'],
      ['Guaranteed income', 'guaranteed-income', 'guaranteed-income-estimator'],
    ];
    ...
    locales.forEach((locale) => {
      calculators.forEach((item) => {
        ...
        cy.intercept(`/${locale}/${embeddedJourney}?isEmbedded=true`).as(
          `${embeddedJourney}`,
        );
        ...
        cy.get(
          `[data-testid='tool-iframe-${embeddedJourney}']`,
        ).scrollIntoView();
        cy.get(`[data-testid='tool-iframe-${embeddedJourney}']`).should(
          'be.visible',
        );
      });
    });
