...
export class BabyCostCalculator {
  elements = {
    ...
    savingLabel: () => cy.get('label[for="q-can-save-i"]'),
    ...
    this.elements.summaryTable().within(() => {
      ...
      if (result) {
        ...
        cy.get('tbody tr')
          ...
          .should(
            'have.text',
            bccData.summary.result.replace('%RESULT%', formattedResult),
          );
      }
    });
  }
  ...
  checkEmailPayload(payload: string) {
    cy.intercept({ method: 'POST', url: '/api/save-and-return' }, (req) => {
  ...
  enterInvalidEmailAndSubmit() {
    cy.get('input[name="email"]').type('notanemail');
    cy.get('button[data-testid="save-and-return"]').click();
  }

  checkErrForInvalidEmail() {
    cy.get('#email').should(
      'have.text',
      'Error: Enter an email address in the correct format, like name@example.com',
    );
    cy.get('#error-summary-heading').should(
      'have.text',
      'Something went wrong',
    );
    cy.get('a[data-testid="error-link-0"]').should(
      'have.text',
