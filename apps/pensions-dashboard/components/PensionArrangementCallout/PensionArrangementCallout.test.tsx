...
import { PensionType, unsupportedPensionTypes } from '../../lib/constants';
import {
  PensionArrangement,
  PhoneNumber,
  PostalAddress,
...
describe('PensionArrangementCallout', () => {
  ...
  it.each([PensionType.DC, PensionType.DB])(
  ...
  it.each(
    Object.values(PensionType).filter((type) =>
      // Add import for unsupportedPensionTypes at the top
      unsupportedPensionTypes.includes(type),
    ),
  )(
    'does not render the pension details link form for %s pension type',
    (pensionType) => {
      render(<PensionArrangementCallout {...{ ...mockdata, pensionType }} />);
      expect(screen.queryByTestId('details-link')).not.toBeInTheDocument();
    },
  );
  ...
  it('renders the preferred contact method', () => {
    render(<PensionArrangementCallout {...mockdata} />);
    expect(
      screen.getByText('(common.contact.preferred) WhatsApp: +123 1111111111'),
    ).toBeInTheDocument();
  });
