...
export const PensionsDashboardLayout = ({
  ...
  // Client-side timeout values to avoid SSR/hydration issues
  const [clientTimeoutDuration, setClientTimeoutDuration] = useState<
    number | null
  >(null);
  const [clientModalTimeoutDuration, setClientModalTimeoutDuration] = useState<
    number | null
  >(null);

  useEffect(() => {
    // Set timeout values on client-side only
    const timeout = TIMEOUT_SECONDS * 1000;
    const modalTimeout = MODAL_TIMEOUT_SECONDS * 1000;
    setClientTimeoutDuration(timeout);
    setClientModalTimeoutDuration(modalTimeout);
  }, []);
        ...
        >
          {title && (
            <Heading
              ...
              className={twMerge(
                'mb-6 text-blue-700 md:mb-8 md:max-w-4xl',
                !isOffset && 'xl:max-w-3xl',
              )}
            >
              {title}
            </Heading>
          ...
          <main id="main">
            ...
            {enableTimeOut &&
              ...
              clientModalTimeoutDuration && (
                <>
                  <Timeout
                    duration={clientTimeoutDuration}
                    modalDuration={clientModalTimeoutDuration}
                    setIsLogoutModalOpen={setIsLogoutModalOpen}
                  />
