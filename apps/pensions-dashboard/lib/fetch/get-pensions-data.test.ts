...
describe('getPensionData', () => {
  ...
  const fullUserSession: UserSession = {
    userSessionId: 'test-session-id',
    authorizationCode: 'test-auth-code',
    scenarioEndPoint: 'test-endpoint',
  ...
  describe('successful data retrieval', () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            ...
            cookie:
              'userSessionId=test-session-id; authorizationCode=test-auth-code; scenarioEndPoint=undefined;',
          }),
    ...
    test('should return pension data with full user session including scenarioEndPoint', async () => {
      (fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
        json: jest.fn().mockResolvedValueOnce(mockPensionData),
      });
      ...
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({
            cookie:
              'userSessionId=test-session-id; authorizationCode=test-auth-code; scenarioEndPoint=test-endpoint;',
          }),
        }),
      );
    });
