...
describe('Pension Data Service', () => {
  ...
  describe('getPensionsSummary', () => {
    ...
    it('should filter out pensions with ERROR category', async () => {
      const mockSummary = {
        isPensionRetrievalComplete: true,
        totalPensionsFound: 2,
        pensions: [
          {
            pei: 'pei-1',
            schemeName: 'Scheme 1',
      ...
      expect(result?.pensions).toHaveLength(2);
      expect(result?.totalPensionsFound).toBe(2);
      expect(result?.pensions.map(({ pei }) => pei)).toEqual([
        'pei-1',
        'pei-3',
      ]);
      expect(
        result?.pensions.find(
          ({ category }) => category === PensionsCategory.ERROR,
        ),
      ).toBeUndefined();
    });
  });
