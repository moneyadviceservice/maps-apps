...
type Props = {
  pageData: DataProps;
  fieldNames: RetirementIncomeFieldTypes[];
...
const RetirementIncomeDetails = ({ content, pageData, fieldNames }: Props) => {
  ...
  const [fields, setFields] =
    useState<RetirementIncomeFieldTypes[]>(fieldNames);
    ...
    <>
      {content.map((partner, partnerIdx) => (
        ...
        >
          ...
          />
          <H3 className="text-blue-800">{partner.partnerName}</H3>
          {partner.content.map((section, sectionIndex) => {
            ...
            return (
              // Does the accordion title need to be H3 ????
              <div
                key={sectionIndex}
              ...
              >
                <ExpandableSection
                  ...
                  open={sectionIndex === 0}
                >
                  <Paragraph className="font-normal pb-2">
