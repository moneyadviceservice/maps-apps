...
jest.mock('@maps-react/hooks/useTranslation', () => {
  const mapping: Record<string, string> = {
    ...
    'saveAndComeBack.partnerEmailLabelText': 'Partner email address',
...
describe('test EmailInputField component', () => {
  ...
  it('renders user label for partnerId 1 and partner label for partnerId 2', () => {
    ...
    expect(screen.getByLabelText('Partner email address')).toBeInTheDocument();
  });
  ...
  it('includes error id in aria-describedby when formErrors is provided even if no specific error message', () => {
    ...
    expect(input.getAttribute('aria-describedby')).toContain('email-hint-1');
    expect(input.getAttribute('aria-describedby')).toContain('email-error-1');
  });
