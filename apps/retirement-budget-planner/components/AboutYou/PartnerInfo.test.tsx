...
describe('PartnerInfo Component', () => {
  ...
  it('shows name field error when provided in formErrors', () => {
    render(
      <PartnerInfo
    ...
    expect(nameError.id).toBe('name-error');
  });

  it('shows dob field error and applies error styling to inputs when dob error present', () => {
    render(
      <PartnerInfo
        partners={basePartners}
        partnerInfo={mockPartner2}
    ...
    const dobError = screen.getByText('Enter a valid date of birth');
    expect(dobError).toBeInTheDocument();
    expect(dobError.id).toBe('dob-error');

    const day = screen.getByLabelText(/day/i);
    const month = screen.getByLabelText(/month/i);
    const year = screen.getByLabelText(/year/i);
    expect(day.className).toMatch(/border-red-700/);
    expect(month.className).toMatch(/border-red-700/);
    expect(year.className).toMatch(/border-red-700/);
  });

  it('shows gender field error when provided in formErrors', () => {
    render(
      <PartnerInfo
        partners={basePartners}
        partnerInfo={mockPartner2}
    ...
    expect(genderError.id).toBe('gender-error');
  });

  it('renders multiple field errors when multiple keys present in formErrors', () => {
    render(
      <PartnerInfo
        partners={basePartners}
        partnerInfo={mockPartner2}
