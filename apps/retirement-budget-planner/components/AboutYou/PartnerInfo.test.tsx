...
jest.mock('lib/validation/partner', () => ({
  ...
  validateName: (
    ...
    // Simple mock validation: return an error object if name is empty, otherwise no errors.
    if (!name || name.trim() === '') {
      return [
        {
          question: 'name',
...
describe('PartnerInfo handleDone', () => {
  beforeEach(() => {
    mockUseTranslation.mockReturnValue({
      t: (key: string) => {
        const translations: { [key: string]: string } = {
  ...
  it('calls onError and does not call updatePartnerInformation when name is valid (validateName returns empty array)', async () => {
    ...
    // validateName returns [] for a valid name in the mocked implementation above,
    // so the component (per current logic) will call onError with that value.
    expect(onError).toHaveBeenCalledWith([]);

    expect(updatePartnerInformation).not.toHaveBeenCalled();
  });

  it('calls onError with validation errors and does not call updatePartnerInformation when name is empty', async () => {
    const onError = jest.fn();
    render(
      <PartnerInfo
        partners={basePartners}
    ...
    const calledArg = onError.mock.calls[0][0];
