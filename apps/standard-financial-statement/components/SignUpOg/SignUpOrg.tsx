...
const SignUpOrg = ({ entry, lang }: { entry: Entry; lang: string }) => {
  ...
  return (
    <>
      <H2 className="my-8 text-5xl">
        {z({
          en: 'Part 1 - Register as an organisation',
          cy: 'Rhan 1 - Cofrestrwch fel sefydliad',
        })}
      </H2>
      <form action="/fn/form-handler" method="POST">
      ...
      <Errors errors={hasError ? ['error'] : []} key={input.name}>
        <div className="mb-8">
          ...
          {hasError && input?.errors && (
            <div className="text-red-500 text-[18px] mb-4 -mt-2">
  ...
  return (
    <Errors errors={hasError ? ['error'] : []} key={input.title}>
      <div className="max-w-2xl mb-8">
        ...
        {hasError && input?.errors && (
          <div className="text-red-500 text-[18px] mb-2 -mt-1">
            {input?.errors[hasError.type]}
          </div>
        )}
...
const InputCheckbox = ({ entry, answer }: { entry: Entry; answer: Answer }) => {
  ...
  useEffect(() => {
    setShowInput(answer?.value !== 'none' && isChecked);
  }, [isChecked, answer]);

  return (
    <div className="max-w-sm">
      <Errors errors={hasError ? ['error'] : []} key={answer.value}>
        <Checkbox
          key={`memberships`}
          name={`memberships`}
          id={answer.value}
          ...
          onChange={(e) => {
            if (e.target.value !== 'none') setShowInput(e.target.checked);
          }}
        >
          {answer.text}
        </Checkbox>
    ...
    <div>
      <Errors errors={hasError ? ['error'] : []}>
        <div key={input.name} className="mb-8" id={`${input.name}`}>
          ...
          {input.description && (
            <p className="mb-4 text-gray-400 -mt-2">{input.description}</p>
          )}
          {hasError && input?.errors && (
