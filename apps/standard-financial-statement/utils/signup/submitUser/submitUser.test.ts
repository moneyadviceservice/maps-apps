...
describe('submitUser', () => {
  ...
  let setShowOTP: jest.Mock;
  let switchFormType: jest.Mock;
  ...
  it('submits successfully and switches to SUCCESS', async () => {
    ...
    expect(switchFormType).toHaveBeenCalledWith(FormType.SUCCESS);
  });
  ...
  it('handles OTP required response from API', async () => {
    ...
    expect(setShowOTP).toHaveBeenCalledWith(true);
    expect(setSubmitButtonDisabled).toHaveBeenCalledWith(false);
    expect(switchFormType).toHaveBeenCalledWith(FormType.OTP);
  });
