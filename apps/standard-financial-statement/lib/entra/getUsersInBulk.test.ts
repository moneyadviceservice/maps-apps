...
describe('getUsersInBulk', () => {
  ...
  const mockSession: IronSessionData = {
  ...
  it('should return Forbidden if user is not authenticated', async () => {
    ...
    const result = await getUsersInBulk(mockSession, emailOrIds);
  ...
  it('should return an error if emailOrIds array is null', async () => {
    ...
    const result = await getUsersInBulk(
      mockSession,
      emailOrIds as unknown as string[],
    );
  ...
  it('should fetch a single user successfully', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          responses: [{ id: '1', status: 200, headers: {}, body: mockUser }],
        }),
    });
          ...
          requests: [
            {
              id: '1',
              method: 'GET',
              url: `/users/user1@example.com?$select=${BATCH_SELECT_USER_PROPERTIES}`,
            },
          ],
        }),
      }),
  ...
  it('should fetch multiple users successfully within a single batch', async () => {
    ...
    mockFetch.mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          responses: [
            { id: '1', status: 200, headers: {}, body: mockUser1 },
            { id: '2', status: 200, headers: {}, body: mockUser2 },
          ],
        }),
    });
    ...
    mockFetch.mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          responses: [
            { id: '1', status: 200, headers: {}, body: mockUser1 },
            { id: '2', status: 404, headers: {}, body: mockErrorBody },
          ],
        }),
    });
  ...
  it('should handle multiple batches correctly', async () => {
    ...
    const result = await getUsersInBulk(mockSession, largeEmailList);
