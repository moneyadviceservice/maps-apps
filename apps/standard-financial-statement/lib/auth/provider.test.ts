...
describe('auth handlers', () => {
    ...
    it('should handle redirect, acquire token, and redirect to state', async () => {
      ...
      const mockMsalInstance = {
        acquireTokenByCode: jest.fn().mockResolvedValue({
          account: { username: 'user', idToken: 'idToken' },
          idTokenClaims: {},
        }),
  ...
  describe('logout', () => {
    it('should destroy session and redirect', async () => {
      ...
      expect(res.redirect).toHaveBeenCalledWith(
        `${msalConfig.auth.authority}your-tenant.onmicrosoft.com/oauth2/v2.0/logout?post_logout_redirect_uri=http://localhost:3000/`,
      );
    });
  });
});
