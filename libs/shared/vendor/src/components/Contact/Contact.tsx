      ...
      {step === 0 && (
        <div className="fixed top-0 right-0">
          <button
            ...
            onClick={() => setStep(1)}
          >
            <div className="flex justify-center t-chat-button-icon md:self-center">
      ...
      {step !== 0 && (
        <>
          ...
          <div
            ...
            className="fixed inset-0 w-screen h-screen bg-gray-800 opacity-25 t-overlay"
            role="none"
            onClick={closeCTA}
          ></div>
        </>
    ...
    <>
      ...
      <ul className="w-full">
        {content.otherPhones.map((v) => {
          return (
            <li key={slug(v.title)} className="flex pb-4">
              <p className="min-w-[7.5rem] w-auto">{v.title + ':'}</p>
              <Link
                href={'tel://' + v.num}
                className="pl-3 text-magenta-500 underline outline-0 hover:no-underline hover:text-pink-900 focus:no-underline focus:text-gray-800 focus:bg-yellow-400 focus:shadow-link-focus"
              >
                {v.num}
              </Link>
            </li>
      ...
      <table className="w-full mt-10 text-base border border-t-0 table-fixed border-slate-400 border-x-0">
        <thead>
          <tr className="border border-t-0 border-slate-400 border-x-0">
          ...
          {content.openHours.map((v) => {
            return (
              ...
              >
                ...
                <td className="pt-2 pb-2 font-bold">{v.value}</td>
              </tr>
            );
          })}
        </tbody>
      </table>
      <p className="w-full my-4">{content.text}</p>
    </>
  );
};
...
const WebChatContact = ({
  ...
  return (
    <>
      ...
      />
      <table className="w-full text-base border border-t-0 table-fixed border-slate-400 border-x-0">
        <thead>
          <tr className="border border-t-0 border-slate-400 border-x-0">
            <th className="text-left">{z({ en: 'Hours', cy: 'Oriau' })}</th>
          </tr>
          ...
          {content.openHours.map((key) => {
            return (
              ...
              >
                ...
                <td className="pt-2 pb-2 font-bold">{key.value}</td>
              </tr>
            );
          })}
        </tbody>
    ...
    <>
      ...
      <div className="flex pb-4">
        ...
        <Link
          ...
          target="_blank"
          className="pl-1 text-pink-800 underline hover:no-underline hover:text-pink-900 focus:no-underline focus:text-gray-800 focus:bg-yellow-400 focus:shadow-link-focus"
        >
          WhatsApp
        </Link>
      </div>
