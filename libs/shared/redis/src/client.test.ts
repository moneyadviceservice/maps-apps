...
describe('getRedisClient', () => {
  ...
  beforeEach(() => {
    ...
    process.env.AZURE_MANAGED_CLIENT_ID = mockTestId;
    process.env.AZURE_MANAGED_CLIENT_SECRET = mockSecret;
    process.env.AZURE_MANAGED_TENANT_ID = mockTenantId;
  });
  ...
  it('throws an error if AZURE_MANAGED_CLIENT_ID is not set', async () => {
    process.env.AZURE_MANAGED_CLIENT_ID = undefined;

    await expect(getRedisClient()).rejects.toThrow(
      'AZURE_MANAGED_CLIENT_ID is empty',
    );
  });

  it('throws an error if AZURE_MANAGED_CLIENT_SECRET is not set', async () => {
    process.env.AZURE_MANAGED_CLIENT_SECRET = undefined;

    await expect(getRedisClient()).rejects.toThrow(
      'AZURE_MANAGED_CLIENT_SECRET is empty',
    );
  });

  it('throws an error if AZURE_MANAGED_TENANT_ID is not set', async () => {
    process.env.AZURE_MANAGED_TENANT_ID = undefined;

    await expect(getRedisClient()).rejects.toThrow(
      'AZURE_MANAGED_TENANT_ID is empty',
    );
  });
});
