      ...
      <div className="space-y-2">
        ...
        {[
          ...
          <div key={item.label} className={twMerge(mainClasses, 'bg-gray-100')}>
            <SummaryRow label={item.label} amount={item.value} />
          </div>
        ))}
        ...
        >
          <SummaryRow label={balanceLabel} amount={balance} />
        </div>
      </div>
    </div>
  );
...
const SummaryRow = ({ label, amount }: { label: string; amount: number }) => {
  return (
    <>
      <Paragraph className="m-0 grow">{label}</Paragraph>
      <NumberFormat
