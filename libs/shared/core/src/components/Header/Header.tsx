...
const SearchForm = ({ language }: { language: string }) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (query) {
      window.location.href = `${form.action}?q=${encodeURIComponent(
        String(query),
      )}`;
    }
  };
...
const Navigation = ({ router, showLanguageSwitcher }: NavProps) => {
  const { z } = useTranslation();
      ...
      >
        <details className="group" open={isSearchOpen} ref={searchRef}>
          <summary
            ...
            } cursor-pointer list-none [&::-webkit-details-marker]:hidden bg-magenta-500 text-white rounded p-1 flex`}
  ...
  return (
    <header data-testid="header">
      ...
      <div className="bg-blue-700 shadow-bottom-gray t-header print:hidden">
        {layout === 'grid' ? (
          <GridContainer>
            <div className="relative z-10 flex items-center col-span-12 py-4">
              {headerContent}
            </div>
          </GridContainer>
        ) : (
          <div className="relative z-10 flex items-center p-4 container-auto">
            {headerContent}
          </div>
        )}
      </div>
