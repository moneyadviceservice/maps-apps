...
import { commonLinkClasses } from '../../components/Link';
import { useTranslation, TranslationGroup } from '@maps-digital/shared/hooks';
...
export const TaskList = ({
  ...
  return (
    <ol
      data-testid={testId}
      className={classNames('border-t-1 border-slate-400', className)}
    >
      {tasks.map((text, idx) => {
          ...
          >
            {enableLinks && !notLaunched ? (
              <form method="POST" noValidate>
                ...
                <button
                  className={classNames(linkClass, gridClass, 'w-full')}
