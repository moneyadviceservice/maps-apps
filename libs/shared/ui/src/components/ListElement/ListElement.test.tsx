...
describe('List component', () => {
  ...
  it('renders the tick-green list correctly', () => {
    render(<ListElement items={Content} color="none" variant="none" />);
    const list = screen.getByTestId('list-element');
    expect(list).toMatchSnapshot();
  });

  it('renders the pros list correctly', () => {
    render(<ListElement items={Content} color="none" variant="pros" />);
    const list = screen.getByTestId('list-element');
    expect(list).toMatchSnapshot();
  });

  it('renders the cons list correctly', () => {
    render(<ListElement items={Content} color="none" variant="cons" />);
    const list = screen.getByTestId('list-element');
    expect(list).toMatchSnapshot();
  });

  it('renders the error list correctly', () => {
    render(<ListElement items={Content} color="red" variant="error" />);
    const list = screen.getByTestId('list-element');
    expect(list).toMatchSnapshot();
  });
});
