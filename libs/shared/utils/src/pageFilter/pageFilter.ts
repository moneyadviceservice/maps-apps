...
export const pageFilter = (
  ...
  const page = pagePrefix ?? 'question';
  ...
  return {
    ...
    goToFirstStep: () =>
      `/${lang}${path}${page}-1?restart=true${addEmbedQuery(isEmbed, '&')}`,
    backStep: (
      currentStep: number,
      storedData: Record<string, RecordValue>,
    ) => {
      ...
      return previousStep > -1
        ? `/${lang}${path}${page}-${previousStep}?${convertQueryParamsToString(
            getDataFromQuery(),
          )}${addEmbedQuery(isEmbed, '&')}`
        : `/${lang}${path}${page}-${currentStep}?${convertQueryParamsToString(
            getDataFromQuery(),
          )}${addEmbedQuery(isEmbed, '&')}`;
    },
    goToSummaryPage: () =>
    ...
    goToLastQuestion: (data: object) => {
      ...
      return `/${lang}${path}${page}-${lastStep}?${convertQueryParamsToString(
        getDataFromQuery(),
      )}${addEmbedQuery(isEmbed, '&')}`;
    },
    ...
    goToQuestion: (question: number) => {
      return `/${lang}${path}${page}-${question}?${convertQueryParamsToString(
        getDataFromQuery(),
      )}${addEmbedQuery(isEmbed, '&')}`;
    },
  };
